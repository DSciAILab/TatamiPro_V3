...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: eventData } = await supabaseAdmin.from('sjjp_events').select('app_id').eq('id', eventId).single();
    ...
    const { error: insertError } = await supabaseAdmin.from('sjjp_athletes').insert(newAthlete);
