...
const AthleteProfileEditForm: React.FC<AthleteProfileEditFormProps> = ({ athlete, onSave, onCancel, mandatoryFieldsConfig }) => {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    try {
      ...
      showError('Failed to update athlete profile.');
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle>Edit Athlete Profile</CardTitle>
        <CardDescription>Update the details for {athlete.firstName} {athlete.lastName}.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Emirates ID Back URL (Optional)</FormLabel>
                  <FormControl>
