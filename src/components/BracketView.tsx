...
const BracketView: React.FC<BracketViewProps> = ({ bracket, allAthletes, division, eventId, isPublic = false, basePath, source }) => {
  ...
  if (!hasRounds) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{division.name}{bracket.group_name ? ` - ${bracket.group_name}` : ''}</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card className="p-4 bg-muted/20">
      <CardHeader>
        <CardTitle className="text-2xl font-bold mb-4">{division.name}{bracket.group_name ? ` - ${bracket.group_name}` : ''}</CardTitle>
      </CardHeader>
