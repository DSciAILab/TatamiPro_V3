...
const AgeDivisionConfig: React.FC<AgeDivisionConfigProps> = ({ settings, onUpdateSettings }) => {
  ...
  const [currentEdit, setCurrentEdit] = useState<Omit<AgeDivisionSetting, 'id'>>({ name: '', min_age: 0 });
  const [newSetting, setNewSetting] = useState<Omit<AgeDivisionSetting, 'id'>>({ name: '', min_age: 0 });

  const sortedSettings = [...settings].sort((a, b) => a.min_age - b.min_age);

  const handleAdd = () => {
    if (!newSetting.name || newSetting.min_age === undefined) {
      showError('Por favor, preencha o nome e a idade mínima.');
      return;
    }
    const updatedSettings = [...settings, { ...newSetting, id: uuidv4() }];
    onUpdateSettings(updatedSettings);
    setNewSetting({ name: '', min_age: 0 });
    showSuccess('Categoria de idade adicionada.');
  };

  const handleEdit = (setting: AgeDivisionSetting) => {
    setEditingId(setting.id);
    setCurrentEdit({ name: setting.name, min_age: setting.min_age });
  };

  const handleSave = () => {
    if (!currentEdit.name || currentEdit.min_age === undefined) {
      showError('O nome e a idade mínima não podem estar vazios.');
      return;
    }
